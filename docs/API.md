...
CUDA autograd is intentionally not exposed as production-ready. Run `python/tests/test_cuda_parity.py` on NVIDIA hardware before enabling CUDA forward in any training or inference path.
...
The compiled wrapper `torch_dbs_extension.final_scores()` accepts `[T,K,V]` and `[B,T,K,V]` on both CPU and CUDA. CPU supports surrogate autograd. CUDA is forward-only and raises on backward. See `docs/PYTORCH_API.md`.
