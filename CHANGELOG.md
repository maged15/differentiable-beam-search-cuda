...
- Added CUDA sparse surrogate backward for the PyTorch CUDA path.
- Made the cooperative CUDA fast kernel the default for supported beam sizes.
- Added CUDA full-forward trace outputs for parents/from-logprob metadata used by sparse backward.
- Added a self-hosted CUDA smoke workflow badge and workflow for install, forward, backward, and parity checks.
- Documented tested PyTorch/CUDA build combinations and added representative benchmark numbers.
